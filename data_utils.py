...
def split_validation(train_set, valid_portion):  # 参数 训练集，验证集比例
    train_set_x, train_set_image, train_set_text, train_set_y = train_set  # 将训练集拆分为四个部分：x, image, text, y
    n_samples = len(train_set_x)# 计算训练集样本数量
    sidx = np.arange(n_samples, dtype='int32') # 生成一个包含所有样本索引的数组
    np.random.shuffle(sidx)# 随机打乱索引数组
    n_train = int(np.round(n_samples * (1. - valid_portion)))# 计算训练集样本数量
    valid_set_x = [train_set_x[s] for s in sidx[n_train:]]# 根据索引数组划分验证集和训练集的x部分
    train_set_x = [train_set_x[s] for s in sidx[:n_train]]
    valid_set_image = [train_set_image[s] for s in sidx[n_train:]]# 根据索引数组划分验证集和训练集的image部分
    train_set_image = [train_set_image[s] for s in sidx[:n_train]]
    valid_set_text = [train_set_text[s] for s in sidx[n_train:]]# 根据索引数组划分验证集和训练集的text部分
    train_set_text = [train_set_text[s] for s in sidx[:n_train]]
    valid_set_y = [train_set_y[s] for s in sidx[n_train:]]# 根据索引数组划分验证集和训练集的y部分
    train_set_y = [train_set_y[s] for s in sidx[:n_train]]

    return (train_set_x, train_set_image, train_set_text, train_set_y), (valid_set_x, valid_set_image, valid_set_text, valid_set_y) # 返回划分后的训练集和验证集


def get_item_num(train, test):#确定物品的个数
...
class Data(Dataset):
    ...
    def _get_max_node_number(self):# 获取最大节点数
        max_node_number = 0
        for idx in range(self.data_length):
            u = self.inputs[idx]
            i = self.image_inputs[idx]
            t = self.text_inputs[idx]
            node_number = len(np.unique(u)) + len(np.unique(i)) + len(np.unique(t))#计算三个数组（u、i、t）中唯一元素的数量之和
            if node_number > max_node_number:
                max_node_number = node_number
        return max_node_number
            

    def _handle_data(self, inputData, train_len=None, reverse=False):# 处理数据，返回处理后的数据和最大长度
    ...
    def replace_with_none(self, m_input, noise_level):# 根据噪声水平替换输入数据中的部分元素为空列表
    ...
    def __getitem__(self, index):
        ...
        """To Build Graph"""
        u_nodes = np.unique(u_input).tolist()# 获取用户输入的唯一节点列表
        i_nodes = np.unique([y for x in image_input for y in x]).tolist()# 获取图像输入的唯一节点列表
        t_nodes = np.unique([y for x in text_input for y in x]).tolist()# 获取文本输入的唯一节点列表
        nodes = u_nodes + i_nodes + t_nodes# 合并三个节点列表
        nodes = np.asarray(nodes + (self.max_len - len(nodes)) * [0]) # return # 如果节点数量不足最大长度，用0填充至最大长度

        u_node_num, i_node_num, t_node_num = len(u_nodes), len(i_nodes), len(t_nodes)# 计算各个节点类型的数量
        node_type_mask = [1]*u_node_num + [2]*i_node_num + [3]*t_node_num# 生成节点类型掩码列表，1表示用户节点，2表示图像节点，3表示文本节点
        node_type_mask = node_type_mask + (self.max_len - len(node_type_mask)) * [0] # return

        adj = np.zeros((self.max_len, self.max_len))# 初始化邻接矩阵

        # relation type # self-loop (1), out (2), in (3), bi-direction (4)
        # relation type # u-i (5), i-u (6), i-t (7), t-i (8), i-t (9), t-i (10)
        for i in np.arange(le):#le: real length of the inputs
        	# item assignment
            item = u_input[i]

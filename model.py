...
class VLGraph(Module):
    def __init__(self, config, img_cluster_feature, txt_cluster_feature, item_image_list, item_text_list):
        ...
        self.batch_size = config['batch_size']# 设置批处理大小
        self.num_item = config['num_node'][config['dataset']]# 获取数据集对应的节点数量
        self.num_image = config['cluster_num'][config['dataset']]# 获取数据集对应的图像聚类数量
        self.num_text = config['cluster_num'][config['dataset']]# 获取数据集对应的文本聚类数量
        self.num_node = self.num_item+self.num_image+self.num_text#计算三种模态的节点总数

        self.dim = config['embedding_size']# 设置嵌入维度
        self.auxiliary_info = config['auxiliary_info']#设置辅助信息，position嵌入以及Node Type
        self.dropout_local = config['dropout_local']# 设置本地dropout率
        self.dropout_atten = config['dropout_atten']# 设置注意力dropout率
        self.n_layer = config['n_layer']

        self.aggregator = config['aggregator']#设置聚合器类型，异构聚合
        self.max_relid = config['max_relid'] # 4/10 # 设置最大关系ID
        ...
        self.embedding = nn.Embedding(self.num_node, self.dim, padding_idx=0)# 创建一个嵌入层，用于将节点的ID映射到低维向量空间中
        self.pos_embedding = nn.Embedding(150, self.dim)# 创建一个嵌入层，用于将位置信息映射到低维向量空间中
        self.node_type_embedding = nn.Embedding(4, self.dim)# 创建一个嵌入层，用于将节点类型映射到低维向量空间中
        # Parameters
        self.w_1 = nn.Parameter(torch.Tensor(2 * self.dim, self.dim))# 定义第一层权重矩阵，大小为(2 * dim, dim)
        self.w_2 = nn.Parameter(torch.Tensor(self.dim, 1))# 定义第二层权重矩阵，大小为(dim, 1)，用于最后的预测输出
        self.w_pos_type = nn.Parameter(torch.Tensor((len(self.auxiliary_info)+1) * self.dim, self.dim))# 定义辅助信息权重矩阵，大小为((len(auxiliary_info)+1) * dim, dim)
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=config['lr_dc_step'], gamma=config['lr_dc'])#学习率调度器

        self.reset_parameters()
        # innitialize embedding
        assert self.num_image == len(img_cluster_feature) and self.num_text == len(txt_cluster_feature)#初始化嵌入矩阵
        # 将图像聚类特征复制到嵌入矩阵的相应位置，并进行归一化处理
        ...
        item_image_dict = torch.tensor(item_image_list, requires_grad=False).cuda() # [I K]# 创建图像索引张量
        self.image_indices = item_image_dict.reshape(-1)
        item_text_dict = torch.tensor(item_text_list, requires_grad=False).cuda() # [I K]# 创建文本索引张量
    ...
    def forward(self, adj, nodes, node_type_mask, node_pos_matrix, stage='train'):
        ...
        if len(self.auxiliary_info)>0:
            ...
            h_nodes = torch.cat(auxiliary_embedding, -1)# 将辅助信息嵌入拼接起来
            h_nodes = torch.matmul(h_nodes, self.w_pos_type)# 通过权重矩阵对辅助信息进行线性变换

        # aggregation
        for i in range(self.n_layer):#在每一层上进行操作，使用局部聚合函数对节点进行聚合
    ...
    def compute_full_scores(self, node_hiddens, alias_item_inputs, alias_img_inputs, alias_txt_inputs, item_seq_mask):
        ...
        item_seq_hiddens = mask * item_seq_hiddens# 将物品序列的隐藏状态与掩码相乘，以屏蔽无效信息
        # 获取物品序列的表示，通过指定的池化方法进行聚合
        ...
        item_emb = self.embedding.weight[1:self.num_item]  # (n_nodes+1) x latent_size# 获取物品嵌入矩阵，从1开始到num_item结束
...
def model_process(model, data, stage='train'):
    ...
    adj = trans_to_cuda(adj).float()# 将各个变量转换为CUDA张量并转换为浮点类型
...
#target通常指的是用户的实际行为数据，例如用户的评分，通过与模型预测的scores进行比较，可以计算出模型的准确性、精确度、召回率等性能指标
#scores则是模型对于推荐项目的预测得分，这些得分可以用来对推荐列表进行排序，从而为用户生成个性化的推荐内容

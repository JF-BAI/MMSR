...
Enviroment Utils
'''
def trans_to_cuda(variable): #是否使用GPU
    if torch.cuda.is_available():
        return variable.cuda()
    else:
        return variable

def trans_to_cpu(variable): # 检查是否有可用的CUDA设备，如果有，将变量从GPU转移到CPU
    if torch.cuda.is_available():
        return variable.cpu()
    else:
        return variable
...
def ensure_dir(dir_path):#确保目录存在
...
class RemoveColorFilter(logging.Filter):
    """
            filter方法接收一个日志记录对象作为参数，并对其进行处理。
            如果日志记录存在，则使用正则表达式移除其中的颜色代码。
            最后返回True表示该日志记录通过过滤。
            """
        ...
        if file_list:
            for file in file_list:
                with open(file, 'r', encoding='utf-8') as f:
                    # 读取文件内容，并使用yaml库的load方法将文件内容转换为字典
                    # 更新file_config_dict字典，将读取到的配置信息添加到字典中

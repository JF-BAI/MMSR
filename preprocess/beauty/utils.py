...
def extract_raw_text_feature(title_corpus, asin_list, tokenizer, model, batch_size=512):
    batch_num = math.ceil(len(title_corpus)/batch_size) #math.ceil()向上取整，求出批次的数量，每个批次大小为512
    ...
    for batch_idx in tqdm(range(batch_num)):
        ...
        if len(features_array) == 0: #检查一个名为features_array的numpy数组是否为空，如果为空，它将sentence_embedding这个张量转换为numpy数组并赋值给features_array，否则将张量转化为numpy数组添加到末尾
...
def build_config(file):
    yaml_loader = yaml.FullLoader# 创建一个yaml解析器对象
    yaml_loader.add_implicit_resolver(
        u'tag:yaml.org,2002:float',# 添加一个隐式解析器，用于处理浮点数的字符串表示形式
        re.compile(
            u'''^(?:
    ...
    file_config_dict = dict()# 创建一个空字典，用于存储配置文件的内容
    if file:
        with open(file, 'r', encoding='utf-8') as f:
            file_config_dict.update(yaml.load(f.read(), Loader=yaml_loader))
    return file_config_dict# 返回配置文件内容的字典
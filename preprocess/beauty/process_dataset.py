...
def get_dataset_folder(config): # TODO
    ...
    if config['dataset_type'] == 'dataset_cf':
        ...
        # 使用Python的字符串格式化功能，将一系列配置参数插入到字符串模板中，生成一个特定的数据集文件夹名称。
...
def basic_filter(rating_df, config):
    ...
    rating_df = rating_df[~(rating_df['rating'] < config['rating_threshold'])]#左侧type是str,右侧=0，int
    # rating_df=rating_df.loc[rating_df['rating'] >= config['rating_threshold']]
    # filter by frequency
    if config['filter_strategy'] == 'core':
        ...
        while del_item_num>0 or del_user_num>0:
            ...
            if del_item_num==0 and del_user_num==0:
                ...
                break
            rating_df = rating_df[~rating_df['itemid'].isin(item_delindexs)]#否定操作符~
...
if __name__=='__main__':
    ...
    dataset_type = config['dataset_type']
    dataset_folder = get_dataset_folder(config)
    if (not config['rewrite']) and os.path.exists(dataset_folder):# 检查config字典中'rewrite'键的值是否为False，并且检查dataset_folder路径是否存在
        print("dataset {} exsit already.".format(dataset_folder))
    else:
        if not os.path.exists(dataset_folder):# 如果不存在，则创建dataset_folder路径'../../ssd1/holdenhu/Amazon2014_dataset/Aamazon_Beauty\\dataset_mmsr\\thre-0.0_filt-core_core-5_split-[比率]_ratio-0.2_imagec-10000_textc-10000_k-3-resnet-t5'
            os.mkdir(dataset_folder)
        if dataset_type == 'dataset_sr':
            build_sr_dataset(config)
        elif dataset_type == 'dataset_mmsr':# 调用build_mmsr_dataset函数，传入config参数

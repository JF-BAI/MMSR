...
#     print('Original #item:', len(set(rating_df['asin'])))
#     print('Original #user:', len(set(rating_df['user_id'])))
#
#
# def get_df(config):  # 获取物品评级数据文件
#     rating_df = pd.read_csv(config['ratingdf_path'], header=None)
#     rating_df.columns = ['user_id', 'timestamp', 'rating', 'asin']  # 原userid', 'itemid','rating','timestamp'
...
#     elif config['dataset_type'] == 'dataset_mmsr':  # 使用config字典中的一些值来格式化字符串，生成dataset_folder
...
#     userid_his = list(rating_df.sort_values(['user_id', 'timestamp'], ascending=True).groupby('user_id'))
#     user_hist_dict = {}
#     for user_record in userid_his:
#         user_id = user_record[0]
#         record = user_record[1]
#         item_list = [itemid_dict[i] for i in list(record['asin'])]
#         t_lit = list(record['timestamp'])
#         user_hist_dict[userid_dict[user_id]] = [(i, t) for i, t in zip(item_list, t_lit)]
#     return user_hist_dict
#
#
# def basic_filter(rating_df, config):  # 进行5-core过滤，去除不满足条件的物品和用户
#     '''
...
#         del_item_num, del_user_num = 100, 100  # 删除10万个物品和用户
#         while del_item_num > 0 or del_user_num > 0:
#             ## drop items
#             item_count = pd.DataFrame(rating_df['asin'].value_counts())  # 原rating_df['itemid']
...
#             ## drop users
#             user_count = pd.DataFrame(rating_df['user_id'].value_counts())  # 原rating_df['userid']
...
#                 break
#             rating_df = rating_df[~rating_df['asin'].isin(item_delindexs)]  # 原rating_df['itemid']
#             rating_df = rating_df[~rating_df['user_id'].isin(user_delindexs)]  # rating_df['userid']
#     elif config['filter_strategy'] == 'user':
#         user_count = pd.DataFrame(rating_df['user_id'].value_counts())  # 原rating_df['userid']
#         user_count.columns = ['nums']
#         user_count = user_count[user_count['nums'] < config['core']]
#         user_delindexs = user_count.index
#         rating_df = rating_df[~rating_df['user_id'].isin(user_delindexs)]
...
#     userid_dict = {}  # to save
#     all_item = list(set(rating_df['asin']))  # rating_df['itemid']
#     all_user = list(set(rating_df['user_id']))
#     for i in all_item:
...
#     print_basic_info(rating_df)
#
#     user_hist_dict = get_user_his_from_df(itemid_dict, userid_dict,
#                                           rating_df)  # each: 30364: [(12197, 1409011200), (7894, 1430524800), (11612, 1430524800)]
#
#     # split train-test
...
#                 if type(asin_image_dict[asin]) == list:
#                     image_c = [c + node_num for c in asin_image_dict[asin]]
#                 else:
#                     image_c = [asin_image_dict[asin] + node_num]  # asin_image_dict[itemid] is an int
#             else:
#                 image_c = []
#             image_seq.append(image_c)
#
#             if asin in asin_text_dict:
#                 if type(asin_text_dict[asin]) == list:
#                     text_c = [c + node_num + image_num for c in asin_text_dict[asin]]
#                 else:
#                     text_c = [asin_text_dict[asin] + node_num + image_num]
#             else:
#                 text_c = []
...
#     readin_feature_path = os.path.join(config['data_root'], 'preprocessed', f_type,
#                                        config['{}_extractor'.format(f_type)],
#                                        'asin-{}-feature{}.npy'.format(source, config[f_type + '_dimension']))
...
#         else:
#             un_found_count += 1
#     print('For {} feature, overall {} items, {} item feature found, {} item feature unfound'.format(f_type, node_num,
#                                                                                                     found_count,
#                                                                                                     un_found_count))
...
#     readin_feature_path = os.path.join(config['data_root'], 'preprocessed', f_type,
#                                        config['{}_extractor'.format(f_type)],
#                                        '{}-center{}-feature{}.npy'.format(f_type, config[f_type + '_cluster_num'],
#                                                                           config[f_type + '_dimension']))
...
#     itemid_dict, userid_dict, all_test_seq, all_train_seq, train, test = build_sr_dataset(config,
#                                                                                           save=False)  # 构建SR数据集，返回itemid_dict, userid_dict, all_test_seq, all_train_seq, train, test
...
#
#     asin_image_dict_path = os.path.join(config['data_root'], 'preprocessed', 'image', config['image_extractor'],
#                                         'asin_image_feature{}_{}_c{}_k{}_dict.npy'.format(image_dimension,
#                                                                                           cluster_method,
#                                                                                           image_cluster_num, link_k))
#     asin_text_dict_path = os.path.join(config['data_root'], 'preprocessed', 'text', config['text_extractor'],
#                                        'asin_text_feature{}_{}_c{}_k{}_dict.npy'.format(text_dimension, cluster_method,
#                                                                                         text_cluster_num, link_k))
#     if not (os.path.exists(asin_image_dict_path) and os.path.exists(asin_text_dict_path)):
#         print("ERROR. asin_image_dict_path [{}] or asin_image_dict_path [{}] not found".format(asin_image_dict_path,
#                                                                                                asin_text_dict_path))
#         return
...
#
#     tra_text_seqs, tra_image_seqs = enrich_data_by_vl(train, node_num, image_num, text_num, reverse_iid_dict,
#                                                       asin_image_dict, asin_text_dict)
#     tes_text_seqs, tes_image_seqs = enrich_data_by_vl(test, node_num, image_num, text_num, reverse_iid_dict,
#                                                       asin_image_dict, asin_text_dict)
#
#     overll_item_img_cluster, overll_item_txt_cluster = get_image_modality_list(node_num, image_num, text_num,
#                                                                                reverse_iid_dict, asin_image_dict,
#                                                                                asin_text_dict, link_k)
#
...
#         pickle.dump(all_train_seq, open(os.path.join(dataset_folder, 'all_train_seq.txt'), 'wb'))
#
#         pickle.dump([train[0], tra_text_seqs, tra_image_seqs, train[1]],
#                     open(os.path.join(dataset_folder, 'train.txt'), 'wb'))
#         pickle.dump([test[0], tes_text_seqs, tes_image_seqs, test[1]],
#                     open(os.path.join(dataset_folder, 'test.txt'), 'wb'))
#
#         pickle.dump(overll_item_img_cluster, open(os.path.join(dataset_folder,
#                                                                'itemid_imageid_list_cluster{}_k{}_feature{}.pickle'.format(
#                                                                    image_cluster_num, link_k,
#                                                                    config['image_dimension'])), 'wb'))
#         pickle.dump(overll_item_txt_cluster, open(os.path.join(dataset_folder,
#                                                                'itemid_textid_list_cluster{}_k{}_feature{}.pickle'.format(
#                                                                    text_cluster_num, link_k, config['text_dimension'])),
#                                                   'wb'))
#
#         pickle.dump(itemid_image_feature, open(
#             os.path.join(dataset_folder, 'itemid_image_feature{}.pickle'.format(config['image_dimension'])), 'wb'))
#         pickle.dump(itemid_text_feature,
#                     open(os.path.join(dataset_folder, 'itemid_text_feature{}.pickle'.format(config['text_dimension'])),
#                          'wb'))
#
#         pickle.dump(image_cluster_feature, open(os.path.join(dataset_folder, 'image_cluster{}_feature{}.pickle'.format(
#             config['image_cluster_num'], config['image_dimension'])), 'wb'))
#         pickle.dump(text_cluster_feature, open(os.path.join(dataset_folder, 'text_cluster{}_feature{}.pickle'.format(
#             config['text_cluster_num'], config['text_dimension'])), 'wb'))
#
#     return
#
#
# if __name__ == '__main__':
#     config = build_config('../../config/preprocess.yaml')  # 创建配置文件
#     dataset_type = config['dataset_type']  # 定义数据集的类型
#     config['data_root'] = '../../ssd1/holdenhu/Amazon2014_dataset/Amazon_Beauty'
#     config[
#         'ratingdf_path'] = '../../ssd1/holdenhu/Amazon2014_dataset/Amazon_Beauty/raw/ratings_Beauty.csv'  # 在运行时，往配置文件中写入ratingdf_path数据
#     working_folder = os.path.join(config['data_root'], dataset_type)
#
#     if not os.path.exists(working_folder):  # 若在Amazon_Beauty下不存在一个dataset_mmsr的文件夹则创建
#         os.makedirs(working_folder)
#
#     dataset_type = config['dataset_type']  # 重复了吧
#     dataset_folder = get_dataset_folder(config)  # 获取数据集文件
...
import pandas as pd
# df = pd.read_csv('../../ssd1/holdenhu/Amazon2014_dataset/Amazon_Beauty/raw/ratings_Beauty.csv')
...
# def get_df(config):
#     rating_df = pd.read_csv(config['../../ssd1/holdenhu/Amazon2014_dataset/Amazon_Beauty/raw/ratings_Beauty.csv'], header=None)
#     rating_df.columns = ['userid','timestamp','rating', 'itemid']
#     print_basic_info(rating_df)
#     return rating_df

rating_df = pd.read_csv('../../ssd1/holdenhu/Amazon2014_dataset/Amazon_Beauty/raw/ratings_Beauty.csv')#,head=None
rating_df.columns = ['itemid','userid','rating','timestamp' ]
print(rating_df[:10])
print(rating_df['rating'].dtypes)
